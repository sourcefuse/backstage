...
import { useRouteRefParams, useRouteRef } from '@backstage/core-plugin-api';
// import { buildRouteRef, jobRunsRouteRef } from '../../../../plugin';
...

  {
    ...
    render: (row: Partial<JobBuild>) => {
      const ActionWrapper = () => {
        ...
        const jobFullName =
          row.fullDisplayName &&
          row.fullDisplayName.split(' #')[0].split(' » ').join('/');
        return (
          <div style={{ width: '98px' }}>
            {row?.url && (
              <Tooltip title="View build">
                <Link
                  to={routeLink({
                    jobFullName: encodeURIComponent(jobFullName as string),
                    buildNumber: String(row?.number),
                  })}
                >
                  <VisibilityIcon />
                </Link>
